...
void main() {
  ...
  // float intensity0 = 1.0 / (1.0 + distanceFromPointLight0 * 7.0); 
  // float intensity1 = 1.0 / (1.0 + distanceFromPointLight1* 7.0);
  float intensity0 = exp(-distanceFromPointLight0 * 0.3) ;   // lower intensity as further away from point light sources
  float intensity1 = exp(-distanceFromPointLight1 * 0.3) ;
